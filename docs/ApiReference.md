...
`ContextMenu` deriva de `System.Windows.Controls.ContextMenu` e preserva `Items`, `ItemContainerStyle`, comandos, bindings, atalhos, teclado, itens marcáveis e submenus nativos. `CornerRadius`, `Padding`, `ItemPadding`, `IconSize`, `IconColumnWidth` e `ArrowColumnWidth` definem a geometria. Cada nível organiza os itens em cinco colunas alinhadas: ícone, título, valor, atalho e seta. `MenuItemAssist.Value` e `MenuItemAssist.ValueTemplate` preenchem a coluna de valor de qualquer item. `Background`, `Foreground`, `BorderBrush`, `HoverBrush`, `CheckedBrush` e `SeparatorBrush` definem as cores. `DisabledOpacity` controla itens indisponíveis. A sombra usa `IsShadowEnabled`, `ShadowColor`, `ShadowOpacity`, `ShadowBlurRadius` e `ShadowDepth`.

`SubmenuPlacementDirection` controla exclusivamente o lado de abertura. Em `Left`, a seta ocupa a primeira coluna, aponta para a esquerda e o submenu usa `PlacementMode.Left`; as demais colunas permanecem na ordem normal. Cada submenu reutiliza o mesmo template, as mesmas cores, geometria e sombra do menu que o abriu.
